...
classify_failure() {
  ...
  if [[ -f "${step_log}" ]]; then
    ...
    if grep -Eqi "readtimeout|connecttimeout|timeoutexception|timed out|request timeout|context deadline exceeded|i/o timeout|maxduration|operation timed out|connection timed out" "${step_log}"; then
...
def is_retryable_connect_error(exc: Exception) -> bool:
    ...
    return (
        'not found' in text
        or 'cannot be resumed' in text
        or 'sandbox' in text and 'resume' in text
    )

def connect_with_retry(sandbox_id, timeout_sec=600, retries=6, base_delay=1.0):
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt == retries or not is_retryable_connect_error(exc):
                raise
            delay = min(base_delay * (2 ** (attempt - 1)), 8.0)
            print(
                f'connect retry {attempt}/{retries} after {type(exc).__name__}: {exc} -> sleep {delay}s'
            )
            time.sleep(delay)

sbx = connect_with_retry(sandbox_id, timeout_sec=600)
